...
MongoDB и тестовое веб-приложение `pymongo_api` здесь разворачиваются с помощью:
`docker-compose.yaml`, который также выполняет парочку скриптов для инициализации:
- `./scripts/init-replica-sh` -- выставляет настройки RS для конфиг-сервера и шардов
- `./scripts/setup-mongos.sh` -- выставляет настройки mongos-роутера.
...
Дальше ставим newman и прогоняем тесты:
